...
def main():
    ...
    if relay_policy:
        if relay_policy_uuid:
            match = mso_template.get_dhcp_relay_policy(existing_dhcp_option_policies, relay_policy_uuid, True)
        else:
            match = mso_template.get_dhcp_relay_policy(existing_dhcp_option_policies, relay_policy, False)
        if match:
